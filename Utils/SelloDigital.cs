...
namespace CFDi40.Utils
{
    public class SelloDigital
    {
        ...
        /// </summary>
        /// <param name="rutaArchivo">Ruta del archivo del certificado</param>
        /// <param name="Inicio"></param>
        /// <param name="Final"></param>
        /// <param name="Serie"></param>
        /// <param name="Numero"></param>
        /// <returns></returns>
        public static Certificado leerCER(string rutaArchivo)
        {
            ...
            {
                throw new Exception("El archivo " + rutaArchivo + " no existe, favor de verificar.");
            }
                X509Certificate2 xCert = new X509Certificate2(rutaArchivo);
            StringBuilder objSB = new StringBuilder("Detalle del certificado: \n\n");

            //Detalle
            objCert.Subject = xCert.Subject;
            objCert.Emisor = xCert.Issuer;
            objCert.ValidoDesde = xCert.NotBefore;
            objCert.ValidoHasta = xCert.NotAfter;
            objCert.TamanioKey = xCert.PublicKey.Key.KeySize.ToString();
            objCert.NoSerie = xCert.SerialNumber;
            serie = xCert.SerialNumber.ToString();

            objSB.AppendLine("Hash = " + xCert.Thumbprint);
            //Numero = "?";
            ...
            objCert.NoCertificado = numero;

            if (DateTime.Now < xCert.NotAfter && DateTime.Now > xCert.NotBefore)
            {
                objCert.Vigente = true;
            }
            else
            {
                objCert.Vigente = false;
            }
